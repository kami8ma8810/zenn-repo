...
このセクションでは、開発中に起きた「期待と実装のズレ」や問題を記録し、同じミスを繰り返さないための教訓をまとめます。

### 2025-11-23: SVGマスクの円の同期実装

#### 問題の経緯

**実装内容**:
- `updateSVGMask()` 関数を作成し、SVG円の位置・サイズをクロップ領域に同期させる機能を実装
- `applyCropResize()`、画像ロード後、リセット後に `updateSVGMask()` を呼び出し
- テストを追加し、テストは成功（✅ 1 passed）

**ユーザーの期待**:
- ハンドルをドラッグしている**最中**もリアルタイムで円が縮む・広がる

**実装の問題点**:
- `applyCropResize()` の**最後**でのみ `updateSVGMask()` を呼び出していた
- ハンドルをドラッグ中（`mousemove` イベント中）には更新されない
- テストは通過したが、**実際のユーザー体験では不完全**だった

**原因分析**:
1. テストが「初期状態での同期」のみを検証していた
2. 「リアルタイム更新」に関するテストが不足していた
3. テストが通過したことで「完了」と判断してしまった
4. 実際のデモでの動作確認を怠った

**教訓**:
- ✅ **テストが通る ≠ 実装完了**
- ✅ テストは「最低限の動作保証」であり、「ユーザー体験の保証」ではない
- ✅ 実装後は必ず実際のデモで動作確認すべき
- ✅ 「リアルタイム性」が求められる機能では、イベント駆動での更新も考慮する

**次のステップ（未実装）**:
- ハンドルの `mousemove` イベントハンドラでも `updateSVGMask()` を呼び出す
- 「ドラッグ中のリアルタイム更新」に関するテストを追加する
...
#### ✅ DO（推奨）
- TDD（Test-Driven Development）でテストファースト
- テスト通過後、**必ず実際のデモで動作確認**
- 「初期状態」だけでなく「動作中の挙動」もテストする
- リアルタイム性が求められる機能では、イベント駆動での更新を考慮

#### ❌ DON'T（非推奨）
- テストが通っただけで「完了」と判断する
- テストだけに頼って、実際のユーザー体験を確認しない
- 静的な状態のテストだけで、動的な挙動のテストを省略する
...
**タイミング**:
- 機能実装が完了したとき
- ユーザーから「期待と違う」と指摘されたとき
- 大きな設計変更があったとき
- セッションの終わりに

**記録内容**:
1. **ユーザーの期待**（何を実現したかったのか）
2. **実装した内容**（何をどう実装したか）
3. **実装の問題点**（期待とのズレ）
4. **原因分析**（なぜズレたのか）
5. **教訓**（次に活かすこと）
...
**記録先**:
- このREADME.mdの「開発履歴と教訓」セクション
- 必要に応じて専用の `DEVELOPMENT_LOG.md` を作成
...
- 機能が「部分的に動作する」段階で中間コミット
- コミットメッセージに「（途中）」などの状態を明記
- 完全に動作するまで `feat:` ではなく `wip:` プレフィックスを検討

### 4. コードレビューの観点

- 実装だけでなく、**ユーザー体験**を重視
- 「動く」だけでなく「気持ちよく動く」かを確認
- リアルタイム性、レスポンシブ性、スムーズさなども評価
